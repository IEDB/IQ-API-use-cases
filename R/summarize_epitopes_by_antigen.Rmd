...
```
Let's define a function for running a query and returning a tibble of results.  Since there may be more than 10K records, we will need to fetch 1 page at a time and continue to append them onto our tibble.  **NOTE:** When paging through results by using an 'offset', it is critical to add an 'order' parameter to ensure that the pages are consistent between queries.
